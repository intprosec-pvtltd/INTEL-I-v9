...
from routers.camera_onboarding import (
...
from services.liveFRS import enabled as remote_frs_enabled, match_live_faces
...
from inference.quality import analyze as analyze_roi_quality
from inference.settings import SETTINGS as CENTRAL_INFERENCE_SETTINGS
...
IS_PROD = ENV == "prod"
PROCESS_ROLE = os.getenv("INTEL_I_PROCESS_ROLE", "api").strip().lower() or "api"
AI_PROCESS = PROCESS_ROLE in {"camera-worker", "analytics-worker"}
...
def kubernetes_liveness_health():
    return liveness_health()


def _runtime_model_readiness_for_current_process() -> dict[str, Any]:
    """Return model readiness without forcing AI graphs into the API process.

    In the distributed architecture the API/control-plane process does not own
    executable AI models. Camera/analytics workers own those graphs, so local
    model verification is not applicable to the API process. AI worker
    availability is checked separately by the startup-health endpoint.

    In an AI process, perform the runtime verification once and then reuse the
    verifier's internal cache on subsequent readiness polls.
    """
    if not AI_PROCESS:
        return {
    ...
    try:
        ...
        return verify_models(force=False)
    except Exception as exc:
        logger.exception("Runtime model readiness verification failed")
        return {
            "status": "NOT_READY",
            "process_role": PROCESS_ROLE,
            "models": [],
...
def readiness_health():
    ...
    model_state = _runtime_model_readiness_for_current_process()
    model_required = os.getenv(
        "MODEL_READINESS_REQUIRED", "false"
    ).strip().lower() in {"1", "true", "yes", "on"} and AI_PROCESS
    models_ok = model_state.get("status") in {"READY", "NOT_APPLICABLE"}
...
def kubernetes_readiness_health():
...
# GPU model ownership belongs to ``runtime.AnalyticsRuntime``.  These names
# remain as compatibility bindings for the existing processFrame function and
# are populated only inside a camera-worker process.
person_model = None
pose_model = None
crime_model = None
ppe_model = None
...
logger.info("Vehicle model configured for worker-owned lazy loading | path=%s", VEHICLE_MODEL_PATH)
...
async def startup_event():
    ...
    # The control plane never loads or verifies executable AI graphs. Model
    # readiness is owned and advertised by camera/ANPR/FRS workers.
    model_state = {"status": "NOT_APPLICABLE", "process_role": PROCESS_ROLE, "models": []}
    model_required = os.getenv(
        "MODEL_READINESS_REQUIRED", "false"
    ).strip().lower() in {"1", "true", "yes", "on"} and AI_PROCESS
    logger.info("AI runtime delegated to workers | process_role=%s", PROCESS_ROLE)
    ...
    if ADVANCED_INTELLIGENCE_ENABLED and AI_PROCESS:
        logger.info(
...
def _person_union_roi(frame, tracked_persons, padding_ratio=0.12):
    """Return a bounded union ROI around tracked persons and its origin.

    Behaviour/PPE models operate on this ROI instead of the full CCTV frame.
    Coordinates are translated back to full-frame space before fusion.
    """
    ...
    h, w = frame.shape[:2]
    try:
        x1 = float(min(box[0] for box in boxes))
        y1 = float(min(box[1] for box in boxes))
        x2 = float(max(box[2] for box in boxes))
        y2 = float(max(box[3] for box in boxes))
    except Exception:
        return None, (0, 0)
    pad_x = max(8.0, (x2 - x1) * float(padding_ratio))
    pad_y = max(8.0, (y2 - y1) * float(padding_ratio))
    ix1=max(0, int(x1-pad_x)); iy1=max(0, int(y1-pad_y))
    ix2=min(w, int(x2+pad_x)); iy2=min(h, int(y2+pad_y))
    if ix2-ix1 < 16 or iy2-iy1 < 16:
        return None, (0, 0)
    return frame[iy1:iy2, ix1:ix2], (ix1, iy1)


def _translate_detection_rows(rows, origin):
    ox, oy = origin
    translated=[]
    for row in rows or []:
        item=dict(row)
        ...
        if isinstance(box, (list, tuple)) and len(box) >= 4:
            ...
            item["center"]=((x1+x2)/2.0+ox,(y1+y2)/2.0+oy)
        translated.append(item)
    return translated


def get_crime_detections_roi(frame, tracked_persons):
    roi, origin = _person_union_roi(frame, tracked_persons)
    if roi is None:
        return []
    return _translate_detection_rows(get_crime_detections(roi), origin)


def get_ppe_detections_roi(frame, tracked_persons, cam_id=None):
    roi, origin = _person_union_roi(frame, tracked_persons, padding_ratio=0.18)
    if roi is None:
        return []
    return _translate_detection_rows(get_ppe_detections(roi, cam_id=cam_id), origin)
...
def _extract_live_faces_from_person_regions(frame, tracked_persons, camID, eligible_track_ids=None):
    ...
    if tracker_ids is None or boxes is None:
        return []
    eligible = None if eligible_track_ids is None else {str(value) for value in eligible_track_ids}

    frame_height,frame_width=frame.shape[:2]
    collected=[]
    for person_index in range(len(tracked_persons)):
        try:
            tracker_id = str(tracker_ids[person_index])
            if eligible is not None and tracker_id not in eligible:
                continue
            ...
            inference_roi=roi
            roi_gate=analyze_roi_quality(roi)
            enhancement=str(roi_gate.get("enhancement") or "original")
            if ADVANCED_INTELLIGENCE_ENABLED and DARKIR_ENABLED and CAMERA_POLICIES.get(str(camID)).darkir and enhancement in {"darkir","darkir+edsr"}:
                try:
                    started=time.perf_counter()
                    dark=advanced_intelligence.restore_low_light(inference_roi, quality={"quality_score": 0.0, "mode": "LOW_LIGHT"})
                    MODEL_METRICS.record_call("darkir", latency_ms=(time.perf_counter()-started)*1000.0)
                    if dark.get("model_used") and isinstance(dark.get("image"),np.ndarray) and dark["image"].size:
                        inference_roi=dark["image"]
                except Exception:
                    MODEL_METRICS.record_call("darkir", error=True)
                    logger.debug("Face ROI DarkIR failed | cam_id=%s track=%s",camID,tracker_id,exc_info=True)
            else:
                MODEL_METRICS.record_avoided("darkir", reason="trigger")
            if ADVANCED_INTELLIGENCE_ENABLED and EDSR_ENABLED and CAMERA_POLICIES.get(str(camID)).edsr and enhancement in {"edsr","darkir+edsr"}:
                try:
                    started=time.perf_counter()
                    sr=advanced_intelligence.upscale_when_needed(inference_roi, 0.0)
                    MODEL_METRICS.record_call("edsr", latency_ms=(time.perf_counter()-started)*1000.0)
                    if sr.get("model_used") and isinstance(sr.get("image"),np.ndarray) and sr["image"].size:
                        inference_roi=sr["image"]
                except Exception:
                    MODEL_METRICS.record_call("edsr", error=True)
                    logger.debug("Face ROI EDSR failed | cam_id=%s track=%s",camID,tracker_id,exc_info=True)
            else:
                MODEL_METRICS.record_avoided("edsr", reason="trigger")

            # Final inexpensive interpolation is only for detector input sizing.
            # Mapping uses independent X/Y scale so any EDSR output remains safe.
            longest=max(inference_roi.shape[1],inference_roi.shape[0])
            extra_scale=min(2.0,max(1.0,640.0/max(1.0,float(longest))))
            if extra_scale>1.05:
                inference_roi=cv2.resize(inference_roi,None,fx=extra_scale,fy=extra_scale,interpolation=cv2.INTER_CUBIC)
            scale_x=float(inference_roi.shape[1])/max(1.0,float(roi_width))
            scale_y=float(inference_roi.shape[0])/max(1.0,float(roi_height))

            _frs_started=time.perf_counter()
            try:
                ...
                MODEL_METRICS.record_call(
                    "frs",
                    latency_ms=(time.perf_counter()-_frs_started)*1000.0,
                    frames=1,
                )
            except Exception:
                MODEL_METRICS.record_call(
            ...
            for face in roi_faces:
                ...
                if not isinstance(local_box,(list,tuple)) or len(local_box)<4:
                    continue
                full_box=[
                    max(0,int(round(float(local_box[0])/scale_x))+roi_x1),
                    max(0,int(round(float(local_box[1])/scale_y))+roi_y1),
                    min(frame_width,int(round(float(local_box[2])/scale_x))+roi_x1),
                    min(frame_height,int(round(float(local_box[3])/scale_y))+roi_y1),
                ]
                ...
                translated['tracker_id']=tracker_id
...
def _run_person_watchlist_match(
    ...
    try:
        ...
        try:
            if remote_frs_enabled():
                observation_ts=(
                    frame_timestamp.replace(tzinfo=timezone.utc).timestamp()
                    if isinstance(frame_timestamp,datetime) and frame_timestamp.tzinfo is None
                    else frame_timestamp.timestamp()
                    if isinstance(frame_timestamp,datetime)
                    else time.time()
                )
                eligible_persons=tracked_persons
                if eligible_track_ids is not None and tracked_persons is not None and getattr(tracked_persons,"tracker_id",None) is not None:
                    eligible_set={str(value) for value in eligible_track_ids}
                    mask=np.array([str(value) in eligible_set for value in tracked_persons.tracker_id],dtype=bool)
                    eligible_persons=tracked_persons[mask]
                _frs_started=time.perf_counter()
                try:
                    faces=match_live_faces(
                        user_id=int(user_id), frame=frame, camera_id=str(camID),
                        timestamp=observation_ts, tracked_persons=eligible_persons,
                    )
                    MODEL_METRICS.record_call(
                        "frs",
                        latency_ms=(time.perf_counter()-_frs_started)*1000.0,
                ...
                if not faces and str(os.getenv("FRS_FULL_FRAME_FALLBACK_ENABLED","false")).strip().lower() in {"1","true","yes","on"}:
                    # Explicit development-only compatibility path. Production
                    # keeps FRS inside tracked person/face ROIs.
                    _frs_fallback_started=time.perf_counter()
                    try:
                        faces=extract_embeddings(frame, profile='live', allow_fallback=True)
                        MODEL_METRICS.record_call(
                            "frs", latency_ms=(time.perf_counter()-_frs_fallback_started)*1000.0, frames=1
                        )
                    except Exception:
                        MODEL_METRICS.record_call(
                            "frs", latency_ms=(time.perf_counter()-_frs_fallback_started)*1000.0, frames=1, error=True
                        )
                        raise
        except RuntimeError:
            logger.exception(
        ...
        for face in faces:
            if face.get('remote_identity_id') is not None:
                entry=entries_by_id.get(str(face.get('remote_identity_id')))
                score=float(face.get('remote_similarity') or 0.0)
            else:
            ...
            global_person_id=None; correlated_track_id=str(face.get('tracker_id') or 'face-watchlist'); correlation_score=0.0
            try:
            ...
            matched_track_ids.add(str(correlated_track_id))
            TRACK_CACHE.mark_attempt(
                str(camID), str(correlated_track_id), "frs",
                value={"entry_id": int(entry.id), "score": float(score)},
        ...
        db.rollback();logger.exception('Person watchlist processing failed | cam_id=%s',camID)
    finally:
        db.close()
...
def processFrame(
    ...
    precomputed_person_detections=None,
    precomputed_vehicle_detections=None,
):
    """
    ...
    try:
        ...
        # Cross-camera identity prefers the normalized evidence timestamp derived
        # from decoder PTS. Some live RTSP feeds do not expose usable PTS on
        # every frame. In non-strict mode, continue analytics with an explicitly
        # marked server-time fallback instead of returning the raw frame before
        # YOLO / tracking / ANPR can run. Source PTS itself is never fabricated.
        if isinstance(frame_timestamp, datetime):
            frame_timestamp_utc = (
        ...
        elif STRICT_PTS_REQUIRED:
            logger.error(
                "Frame rejected because PTS evidence timestamp is unavailable | cam_id=%s",
                camID,
            )
            return frame
        else:
            ...
            normalized_source = str(timestamp_source or "").strip().upper()
            normalized_quality = str(timestamp_quality or "").strip().upper()

            if normalized_source in {"", "UNKNOWN", "NONE"}:
                timestamp_source = "SERVER_PROCESS_TIME"
            if normalized_quality in {"", "UNKNOWN", "NONE"}:
                timestamp_quality = "ESTIMATED"

            logger.debug(
                "PTS evidence timestamp unavailable; using server processing time "
                "for live analytics | cam_id=%s timestamp_source=%s "
                "timestamp_quality=%s",
                camID,
                timestamp_source,
                timestamp_quality,
        ...
        if ADVANCED_INTELLIGENCE_ENABLED:
            try:
                darkir_primary_enabled = str(os.getenv("DARKIR_PRIMARY_DETECTION_ENABLED", "false")).strip().lower() in {"1","true","yes","on"}
                if DARKIR_ENABLED and low_light_frame and darkir_primary_enabled:
                    if (
                        float(quality.get("quality_score", 1.0))
                        <= DARKIR_MIN_QUALITY
                    ):
                        _darkir_started = time.perf_counter()
                        result = advanced_intelligence.restore_low_light(
                            frame,
                            quality=quality,
                        )
                        MODEL_METRICS.record_call("darkir", latency_ms=(time.perf_counter()-_darkir_started)*1000.0)
                ...
                if DARKIR_STRICT and low_light_frame and DARKIR_ENABLED and darkir_primary_enabled:
                    raise
            if DARKIR_ENABLED and not (low_light_frame and darkir_primary_enabled):
                MODEL_METRICS.record_avoided("darkir", reason="trigger")
        ...
        camera_ai_policy = CAMERA_POLICIES.get(str(camID))

        # Centralized inference can inject true cross-camera batched primary
        # detections.  When present, processFrame MUST NOT invoke YOLO again.
        if not camera_ai_policy.object_detection:
            MODEL_METRICS.record_avoided("primary_detector", reason="trigger")
        elif precomputed_person_detections is not None or precomputed_vehicle_detections is not None:
            person_detections = (
                precomputed_person_detections
                if precomputed_person_detections is not None
                else sv.Detections.empty()
            )
            vehicle_detections = (
                precomputed_vehicle_detections
                if precomputed_vehicle_detections is not None
                else sv.Detections.empty()
            )
        else:
            # Compatibility/local mode.  Central production mode performs this
            # work once for the entire DynamicBatcher batch in AnalyticsRuntime.
            _primary_started = time.perf_counter()
            try:
                with model_lock:
            ...
            try:
                if VEHICLE_DETECTOR_MODE == "shared":
                    with model_lock:
                        ...
                        vehicle_detections = sv.Detections.from_ultralytics(shared_result)
                else:
                    ...
                    try:
                        ...
                        for _class_id in (vehicle_detections.class_id if vehicle_detections.class_id is not None else []):
                            vehicle_detections_total.labels(
                                str(camID),
                                str(class_names.get(int(_class_id), _class_id))[:64],
                            ).inc()
                    except Exception:
                        pass
            except Exception:
            ...
            MODEL_METRICS.record_call(
                "primary_detector",
                latency_ms=(time.perf_counter() - _primary_started) * 1000.0,
            )
        ...
        # ------------------------------------------------------------
        crime_detections = []
        existing_camera_alerts = activeAlertBoxes.get(str(camID), {}) or {}
        behaviour_decision = BEHAVIOUR_GATE.decide(
            str(camID),
            tracked_persons,
            frame_width=frame_w,
            frame_height=frame_h,
            restricted=bool(is_camera_restricted),
            active_alert=bool(existing_camera_alerts),
            policy=camera_ai_policy,
        )
        run_behavior_models = bool(behaviour_decision.run_behaviour)

        if run_behavior_models:
            _behaviour_started = time.perf_counter()
            try:
                crime_detections = get_crime_detections_roi(frame, tracked_persons)
                MODEL_METRICS.record_call(
                    "behaviour", latency_ms=(time.perf_counter()-_behaviour_started)*1000.0
                )
            except Exception:
                MODEL_METRICS.record_call(
                    "behaviour", latency_ms=(time.perf_counter()-_behaviour_started)*1000.0, error=True
                )
                logger.exception(
            ...
            MODEL_METRICS.record_avoided("behaviour", reason="trigger")

        # ------------------------------------------------------------
        # PPE / FACE-MASK DETECTION — person/face gated ROI only
        # ------------------------------------------------------------
        ppe_detections = []
        run_ppe_model = bool(
            run_behavior_models and len(tracked_persons) > 0
            and (camera_ai_policy.mask or camera_ai_policy.behaviour)
        )
        if run_ppe_model:
            _mask_started = time.perf_counter()
            try:
                ppe_detections = get_ppe_detections_roi(frame, tracked_persons, cam_id=camID)
                MODEL_METRICS.record_call(
                    "mask", latency_ms=(time.perf_counter()-_mask_started)*1000.0
                )
            except Exception:
                MODEL_METRICS.record_call(
                    "mask", latency_ms=(time.perf_counter()-_mask_started)*1000.0, error=True
                )
                logger.exception("PPE detection failed | cam_id=%s", camID)
                ppe_detections = []
        else:
            MODEL_METRICS.record_avoided("mask", reason="trigger")

        # ------------------------------------------------------------
        # PERSON POSE — only when behavioural gating actually requires it
        # ------------------------------------------------------------
        pose_result = None
        if behaviour_decision.run_pose and len(tracked_persons) > 0:
            _pose_started = time.perf_counter()
            try:
                with model_lock:
                    pose_result = pose_model(
                        frame, conf=POSE_CONF, device=DEVICE, verbose=False,
                    )[0]
                MODEL_METRICS.record_call(
                    "pose", latency_ms=(time.perf_counter()-_pose_started)*1000.0
                )
            except Exception:
                MODEL_METRICS.record_call(
                    "pose", latency_ms=(time.perf_counter()-_pose_started)*1000.0, error=True
                )
                logger.exception("Pose detection failed | cam_id=%s", camID)
                pose_result = None
        else:
            MODEL_METRICS.record_avoided("pose", reason="trigger")
        ...
        if (
            ...
            for index in range(len(tracked_persons)):
                ...
                if ADVANCED_INTELLIGENCE_ENABLED and PERSON_APPEARANCE_ENABLED and camera_ai_policy.person_reid:
                    try:
                        key = f"{camID}:{current_track_id}"
                        person_crop = frame[max(0, y1):min(frame_h, y2), max(0, x1):min(frame_w, x2)]
                        if person_crop.size:
                            crop_gate = analyze_roi_quality(person_crop)
                            area_ratio = float(person_crop.shape[0] * person_crop.shape[1]) / max(1.0, float(frame_h * frame_w))
                            person_quality = max(0.0, min(1.0, 0.55 * min(1.0, area_ratio * 12.0) + 0.45 * min(1.0, float(crop_gate.get("blur",0.0))/120.0)))
                            run_person_reid, reid_reason = TRACK_CACHE.should_run(
                                str(camID), current_track_id, "person_reid",
                                quality=person_quality,
                                refresh_seconds=CENTRAL_INFERENCE_SETTINGS.reid_interval_seconds,
                                success_refresh_seconds=max(6.0, CENTRAL_INFERENCE_SETTINGS.reid_interval_seconds * 3.0),
                            )
                            if run_person_reid:
                                _person_reid_started=time.perf_counter()
                                appearance=appearance_features(person_crop)
                                MODEL_METRICS.record_call("person_reid", latency_ms=(time.perf_counter()-_person_reid_started)*1000.0)
                                personAppearanceStates[key]=appearance
                                personAppearanceLastFrame[key]=current_frame_count
                                person_identity=None
                                try:
                                    person_confidence=(tracked_persons.confidence[index] if tracked_persons.confidence is not None else 0.0)
                                    person_identity=_correlate_and_persist_person(
                                        cam_id=camID, track_id=current_track_id, bbox=person_box,
                                        confidence=person_confidence, appearance=appearance,
                                ...
                                TRACK_CACHE.mark_attempt(
                                    str(camID), current_track_id, "person_reid", quality=person_quality,
                                    value=personAppearanceStates.get(key), successful=bool(appearance), ttl_seconds=30.0,
                                )
                            else:
                                MODEL_METRICS.record_avoided("person_reid", reason="cache")
                    except Exception:
                        MODEL_METRICS.record_call("person_reid", error=True)
                        logger.debug("Person appearance extraction failed | cam_id=%s track=%s",camID,current_track_id,exc_info=True)
                else:
                    MODEL_METRICS.record_avoided("person_reid", reason="trigger")
                ...
                for alert in person_alerts:
                    ...
                    allowed, reason = final_alert_allowed(alert, zone_result)
                    if not allowed:
                        logger.debug("Alert rejected | cam_id=%s rule=%s reason=%s", camID, rule, reason)
                        continue
        ...
        # ------------------------------------------------------------
        # FRS — tracked-person, quality and cache triggered
        # ------------------------------------------------------------
        eligible_frs_tracks=set()
        if camera_ai_policy.frs and tracked_persons is not None and len(tracked_persons)>0 and getattr(tracked_persons,"tracker_id",None) is not None:
            for _idx in range(len(tracked_persons)):
                try:
                    _tid=str(tracked_persons.tracker_id[_idx])
                    _box=[int(v) for v in tracked_persons.xyxy[_idx][:4]]
                    _x1,_y1,_x2,_y2=_box
                    _roi=frame[max(0,_y1):min(frame_h,_y2),max(0,_x1):min(frame_w,_x2)]
                    if _roi.size==0:
                        continue
                    _q=analyze_roi_quality(_roi)
                    _quality=max(0.0,min(1.0,0.6*min(1.0,float(_q.get("blur",0.0))/120.0)+0.4*min(1.0,float(_roi.shape[0]*_roi.shape[1])/max(1.0,float(frame_h*frame_w))*15.0)))
                    _run,_reason=TRACK_CACHE.should_run(
                        str(camID),_tid,"frs",quality=_quality,
                        refresh_seconds=max(0.1,1.0/max(0.1,CENTRAL_INFERENCE_SETTINGS.frs_max_fps)),
                        success_refresh_seconds=float(os.getenv("FRS_TRACK_REFRESH_SECONDS","30")),
                    )
                    if _run:
                        eligible_frs_tracks.add(_tid)
                    else:
                        MODEL_METRICS.record_avoided("frs",reason="cache")
                except Exception:
                    logger.debug("FRS eligibility failed | cam_id=%s",camID,exc_info=True)
        elif not camera_ai_policy.frs or tracked_persons is None or len(tracked_persons)==0:
            MODEL_METRICS.record_avoided("frs",reason="trigger")

        if eligible_frs_tracks:
            matched_frs_tracks=_run_person_watchlist_match(
                frame,camID,source_type,tracked_persons,
                frame_timestamp=frame_timestamp_utc.replace(tzinfo=None),
                timestamp_source=timestamp_source,timestamp_quality=timestamp_quality,
                snapshot_frame=clean_evidence_frame,eligible_track_ids=eligible_frs_tracks,
            ) or set()
            for _tid in eligible_frs_tracks:
                if str(_tid) not in {str(v) for v in matched_frs_tracks}:
                    _existing=TRACK_CACHE.get(str(camID),_tid,"frs")
                    TRACK_CACHE.mark_attempt(
                        str(camID),_tid,"frs",quality=float(_existing.quality if _existing else 0.0),
                        value=None,successful=False,ttl_seconds=15.0,
                    )
        ...
        if not vehicle_tracks:
            MODEL_METRICS.record_avoided("vehicle_reid",reason="trigger")
            MODEL_METRICS.record_avoided("anpr_detector",reason="trigger")
            MODEL_METRICS.record_avoided("ocr",reason="trigger")
        ...
        for vehicle in vehicle_tracks:
            ...
            cheap_quality = analyze_roi_quality(original_vehicle_crop)
            enhancement_decision = str(cheap_quality.get("enhancement") or "original")

            reid_should_run, reid_trigger_reason = TRACK_CACHE.should_run(
                str(camID), track_id, "vehicle_reid",
                quality=roi_quality_score,
                refresh_seconds=CENTRAL_INFERENCE_SETTINGS.reid_interval_seconds,
                success_refresh_seconds=max(8.0, CENTRAL_INFERENCE_SETTINGS.reid_interval_seconds * 4.0),
            ) if camera_ai_policy.vehicle_reid else (False, "policy_disabled")
            anpr_should_run, anpr_trigger_reason = TRACK_CACHE.should_run(
                str(camID), track_id, "anpr",
                quality=roi_quality_score,
                refresh_seconds=max(0.1, 1.0/max(0.1, CENTRAL_INFERENCE_SETTINGS.anpr_max_fps)),
                success_refresh_seconds=float(os.getenv("ANPR_TRACK_SUCCESS_REFRESH_SECONDS", "30")),
            ) if camera_ai_policy.anpr else (False, "policy_disabled")
            cached_reid = TRACK_CACHE.get(str(camID), track_id, "vehicle_reid")
            cached_anpr = TRACK_CACHE.get(str(camID), track_id, "anpr")
            should_run_vehicle_identity = bool(reid_should_run or anpr_should_run)

            # DarkIR is ROI-only by default.  The original ROI remains canonical
            # evidence; only the downstream inference copy may be enhanced.
            if (
                anpr_should_run and ADVANCED_INTELLIGENCE_ENABLED and DARKIR_ENABLED
                and camera_ai_policy.darkir
                and enhancement_decision in {"darkir", "darkir+edsr"}
            ):
                try:
                    _darkir_started = time.perf_counter()
                    dark = advanced_intelligence.restore_low_light(anpr_crop, quality=roi_quality)
                    MODEL_METRICS.record_call("darkir", latency_ms=(time.perf_counter()-_darkir_started)*1000.0)
                    if dark.get("model_used") and isinstance(dark.get("image"), np.ndarray) and dark["image"].size:
                        anpr_crop = dark["image"]
                except Exception:
                    MODEL_METRICS.record_call("darkir", error=True)
                    logger.debug("Vehicle ANPR DarkIR enhancement failed | cam_id=%s track=%s", camID, track_id, exc_info=True)
            else:
                MODEL_METRICS.record_avoided("darkir", reason="trigger")

            edsr_candidate = (
                anpr_should_run and ADVANCED_INTELLIGENCE_ENABLED and EDSR_ENABLED and camera_ai_policy.edsr
                and (
                    enhancement_decision in {"edsr", "darkir+edsr"}
                    or roi_quality_score < EDSR_MIN_QUALITY or roi_w < 420 or roi_h < 220
                )
            )
            if edsr_candidate:
                try:
                    _edsr_started = time.perf_counter()
                    sr = advanced_intelligence.upscale_when_needed(anpr_crop, roi_quality_score)
                    MODEL_METRICS.record_call("edsr", latency_ms=(time.perf_counter()-_edsr_started)*1000.0)
                    if sr.get("model_used") and isinstance(sr.get("image"), np.ndarray) and sr["image"].size:
                        anpr_crop = sr["image"]
                except Exception:
                    MODEL_METRICS.record_call("edsr", error=True)
            ...
            # --------------------------------------------------------
            # RE-ID — new/better/reacquired/refresh-triggered per track
            # --------------------------------------------------------
            reid_quality = float(cached_reid.quality if cached_reid else 0.0)
            if cached_reid is not None and cached_reid.successful and isinstance(cached_reid.value, dict):
                embedding = cached_reid.value.get("embedding")
            if reid_should_run:
                try:
                    _reid_started = time.perf_counter()
                    vehicle_reid_attempts_total.inc()
                    reid_quality_probe = get_vehicle_embedding_result(reid_crop)
                    _reid_elapsed=(time.perf_counter()-_reid_started)*1000.0
                    reid_inference_seconds.observe(_reid_elapsed/1000.0)
                    MODEL_METRICS.record_call("vehicle_reid",latency_ms=_reid_elapsed)
                    reid_quality = _safe_float(reid_quality_probe.get("crop_quality"),0.0,0.0,1.0) if isinstance(reid_quality_probe,dict) else 0.0
                    embedding = (reid_quality_probe.get("embedding") if isinstance(reid_quality_probe,dict) and reid_quality >= VEHICLE_REID_MIN_CROP_QUALITY else None)
                    TRACK_CACHE.mark_attempt(
                        str(camID),track_id,"vehicle_reid",quality=reid_quality,
                        value={"embedding":embedding} if embedding is not None else None,
                        successful=embedding is not None,ttl_seconds=30.0,
                    )
                except Exception:
                    MODEL_METRICS.record_call("vehicle_reid",error=True)
                    embedding = cached_reid.value.get("embedding") if cached_reid and cached_reid.successful and isinstance(cached_reid.value,dict) else None
            else:
                MODEL_METRICS.record_avoided("vehicle_reid",reason="cache" if reid_trigger_reason=="cache" else "trigger")
            ...
            if cached_anpr is not None and cached_anpr.successful and isinstance(cached_anpr.value,dict):
                plate_result.update(cached_anpr.value)
            ...
            if anpr_should_run:
                try:
                    ...
                    _anpr_elapsed=(time.perf_counter()-_anpr_started)*1000.0
                    if isinstance(result, dict):
                        plate_result.update(result)
                    _plate_ok=bool(plate_result.get("plate"))
                    TRACK_CACHE.mark_attempt(
                        str(camID),track_id,"anpr",quality=roi_quality_score,
                        value=dict(plate_result) if _plate_ok else None,successful=_plate_ok,ttl_seconds=45.0,
                    )
                    ...
                    if ANPR_TERMINAL_LOGGING_ENABLED:
                        logger.exception(
                            "[ANPR] processing failed | camera=%s track=%s",camID,track_id,
                        )
            else:
                MODEL_METRICS.record_avoided("anpr_detector",reason="cache" if anpr_trigger_reason=="cache" else "trigger")
                MODEL_METRICS.record_avoided("ocr",reason="cache" if anpr_trigger_reason=="cache" else "trigger")
    ...
    try:
        ...
        while not stop_event.is_set():
            ...
            if not ok or frame is None:
                ...
                continue
            if source_type == "upload" and playback_clock.wait(
                stop_event, decoded.pts_seconds, (getattr(cap, "metadata", {}) or {}).get("fps")
            ):
                break
            ...
            # --------------------------------------------------------
            # EFFECTIVE FRAME TIMESTAMP / PROVENANCE
            # --------------------------------------------------------
            # Prefer MediaTimeline's PTS-derived UTC timestamp. For streams that
            # decode correctly but do not expose usable source PTS (for example,
            # some live Sentinel RTSP feeds), non-strict mode falls back to the
            # actual server receive time. The fallback is explicitly labelled as
            # estimated and ``pts_seconds`` remains None, so evidence provenance
            # never claims that server time came from the camera/container.
            effective_source_timestamp = timing.normalized_timestamp
            effective_timestamp_source = timing.timestamp_source
            effective_timestamp_quality = timing.timestamp_quality

            if not isinstance(effective_source_timestamp, datetime):
                if strict_pts:
                    camera_health_registry.decoder_failure(
                        camID,
                        message="PTS-derived evidence timestamp unavailable",
                    )
                    _set_camera_connection_state(camID, "DEGRADED")
                    logger.error(
                        "Strict PTS policy rejected frame without evidence timestamp "
                        "| cam_id=%s source_type=%s",
                        camID,
                        source_type,
                    )
                ...
                if isinstance(received_at, datetime):
                    effective_source_timestamp = received_at
                    effective_timestamp_source = "SERVER_RECEIVE_TIME"
                else:
                    effective_source_timestamp = datetime.now(timezone.utc)
                    effective_timestamp_source = "SERVER_PROCESS_TIME"

                effective_timestamp_quality = "ESTIMATED"

                logger.debug(
                    "Source PTS timestamp unavailable; using server-time fallback "
                    "| cam_id=%s source_type=%s timestamp_source=%s",
                    camID,
                    source_type,
            ...
            packet = FramePacket(
                ...
                timestamp_quality=effective_timestamp_quality,
...
def system_health(
    ...
    # Central inference telemetry is intentionally fetched by the API so the
    # frontend never needs direct access to the internal GPU service.
    if os.getenv("CENTRAL_INFERENCE_ENABLED", "false").strip().lower() in {"1", "true", "yes", "on"}:
        try:
            import httpx
            inference_url = os.getenv("INFERENCE_SERVICE_URL", "http://127.0.0.1:9300").rstrip("/")
            headers={}
            inference_token=os.getenv("INFERENCE_INTERNAL_TOKEN","").strip()
            if inference_token:
                headers["X-Intel-I-Inference-Token"]=inference_token
            with httpx.Client(timeout=1.5,headers=headers) as client:
                response = client.get(f"{inference_url}/health/ready")
                response.raise_for_status()
                payload["inference"] = response.json()
            central_gpu=(payload.get("inference") or {}).get("gpu") or {}
            if central_gpu:
                used=float(central_gpu.get("memory_used_mb") or 0.0)
                total=float(central_gpu.get("memory_total_mb") or 0.0)
                payload.setdefault("ai",{})["gpu"]={
                    "available":bool(central_gpu.get("cuda_available")),
                    "name":central_gpu.get("name"),
                    "utilization_percent":central_gpu.get("utilization_percent"),
                    "memory_used_mb":used,"memory_total_mb":total,
                    "memory_percent":round((used/total)*100.0,2) if total>0 else None,
                    "temperature_c":central_gpu.get("temperature_c"),
                    "decoder_utilization_percent":central_gpu.get("decoder_utilization_percent"),
                    "source":"central_inference_worker",
                }
        except Exception as exc:
            payload["inference"] = {"status": "UNAVAILABLE", "error": f"{type(exc).__name__}: {str(exc)[:160]}"}
    if DISTRIBUTED_CAMERA_WORKERS_ENABLED:
        try:
...
def startup_health(
    ...
    model_state = _runtime_model_readiness_for_current_process()
    model_required = env_enabled("MODEL_READINESS_REQUIRED", IS_PROD) and AI_PROCESS
    models_ready = model_state.get("status") in {"READY", "NOT_APPLICABLE"}
    scheduler_state = ai_scheduler.status()

    # The API/control-plane process intentionally does not run the local AI
    # scheduler or load executable AI graphs when distributed camera workers
    # are enabled. In that mode, a live distributed worker heartbeat is the
    # operational proof that the AI execution plane is available.
    distributed_ai_required = (
        DISTRIBUTED_CAMERA_WORKERS_ENABLED
        and env_enabled("AI_WORKER_READINESS_REQUIRED", IS_PROD)
    )
    distributed_worker_count = int(camera_state.get("active_workers") or 0)

    if AI_PROCESS:
        scheduler_ready = bool(scheduler_state.get("running"))
        ai_ready = bool(scheduler_ready and models_ready)
        ai_required = model_required
        if ai_ready:
            ai_detail = "AI scheduler running; enabled models verified"
        elif not models_ready:
            ai_detail = "One or more enabled AI models are not verified"
        else:
            ai_detail = "AI scheduler is not running in the AI process"
    elif DISTRIBUTED_CAMERA_WORKERS_ENABLED:
        scheduler_ready = True  # Scheduler/model ownership is delegated.
        ai_ready = distributed_worker_count > 0
        ai_required = distributed_ai_required
        if ai_ready:
            ai_detail = (
                "AI runtime delegated to distributed workers; "
                f"{distributed_worker_count} worker(s) active"
            )
        else:
            ai_detail = (
                "AI runtime delegated to distributed workers; "
                "waiting for a worker heartbeat"
            )
    else:
        scheduler_ready = False
        ai_ready = False
        ai_required = False
        ai_detail = "AI runtime is not active in the API process"
...
async def streamUpload(
    ...
    """Stream an uploaded video through the same distributed worker plane as live cameras.

    In distributed mode the API never opens the video or starts a local AI thread.
    It persists RUNNING intent and proxies MJPEG from the worker that owns the
    upload camera.  Legacy in-process processing remains available only when
    distributed camera workers are explicitly disabled.
    """
    ...
    with SessionLocal() as db:
        ...
        # Keep upload lookup metadata available to worker-side source resolvers
        # and the API's ownership/audit helpers without opening the media here.
        set_upload_state(
            stream_id=streamID,
            cam_id=uploaded_cam_id,
        ...
        if DISTRIBUTED_CAMERA_WORKERS_ENABLED:
            ...
            _record_audit_event(
                db,
                user_id=user_id,
                action="UPLOAD_PROCESSING_REQUESTED",
                resource_type="uploaded_video",
                resource_id=streamID,
                details={
                    "cam_id": uploaded_cam_id,
            ...
            return StreamingResponse(
                stream_distributed_camera(camera_pk, uploaded_cam_id, request),
                media_type="multipart/x-mixed-replace; boundary=frame",
            )

    # ------------------------------------------------------------------
    # Legacy single-process fallback.
    # This path is intentionally unreachable in distributed deployments.
    # ------------------------------------------------------------------
    try:
    ...
    # Legacy mode requires an AI-owning process role because the API role does
    # not load models or start the scheduler by design.
    if not AI_PROCESS:
        raise HTTPException(
            status_code=503,
            detail=(
                "Legacy upload processing requires INTEL_I_PROCESS_ROLE="
                "analytics-worker or distributed camera workers"
            ),
        )
...
def startCamera(
    ...
    allowed_source_types = {
        "rtsp", "http", "https", "live", "hls", "webcam",
        "upload", "recorded_video", "onvif", "nvr", "vms",
        "vendor_api", "vendor_sdk",
    }
    if source_type not in allowed_source_types:
        raise HTTPException(
            status_code=400,
            detail=f"Unsupported camera source type: {source_type}",
        )

    # Production/distributed path: the API is control-plane only.  Persist the
    # requested state and let a camera worker acquire the Redis lease, resolve
    # the source, load AnalyticsRuntime and own inference.
    if DISTRIBUTED_CAMERA_WORKERS_ENABLED:
        set_camera_desired_state(db, camera, "RUNNING", commit=True)
        _record_audit_event(
            db,
    ...
    # ------------------------------------------------------------------
    # Legacy single-process fallback only.
    # ------------------------------------------------------------------
    if not AI_PROCESS:
        raise HTTPException(
            status_code=503,
            detail=(
                "In-process camera analytics requires INTEL_I_PROCESS_ROLE="
                "analytics-worker or enable INTEL_I_DISTRIBUTED_CAMERA_WORKERS"
            ),
        )
    ...
    if source_type in {"upload", "recorded_video"}:
        uploaded = get_uploaded_video_by_cam(
            db=db,
            cam_id=camID,
            user_id=current_user.id,
        ...
        try:
            source = resolve_uploaded_video_path(str(uploaded.storage_key))
        except FileNotFoundError as exc:
            raise HTTPException(status_code=404, detail="Uploaded video object missing") from exc
        ...
        source = str(source)
    elif source_type in {"rtsp", "http", "https", "live", "hls"}:
        encrypted_source = getattr(camera, "encrypted_source", None)
        if not encrypted_source:
            raise HTTPException(status_code=500, detail="Camera source is not configured")
        try:
            ...
            logger.exception("Failed to decrypt camera source | cam_id=%s", camID)
            raise HTTPException(status_code=500, detail="Unable to access camera source") from exc
    elif source_type == "webcam":
        encrypted_source = getattr(camera, "encrypted_source", None)
        if not encrypted_source:
            raise HTTPException(status_code=500, detail="Webcam source is not configured")
        try:
            source = int(str(decrypt_camera_source(encrypted_source)).strip())
        except (TypeError, ValueError) as exc:
            raise HTTPException(status_code=400, detail="Invalid webcam device ID") from exc
        if source < 0:
            raise HTTPException(status_code=400, detail="Webcam device ID cannot be negative")
    else:
        encrypted_config = getattr(camera, "connector_config_encrypted", None)
        ...
        try:
            connector_config = decrypt_connector_config(encrypted_config)
            validate_connector_config(source_type, connector_config)
            from connectors.manager import resolve_connector
            result = resolve_connector(source_type, connector_config)
            # Preserve the legacy execution contract: start_camera_worker()
            # receives the validated connector config and resolves it through
            # the existing connector path. The preflight result is persisted
            # only as metadata.
    ...
    if camera.zones:
        zone_manager.set_zones(camID, camera.zones)
    camera_config.set_mode(camID, camera.zone)

    if not already_running:
        active_count = get_active_live_camera_count()
        capacity_check = can_start_camera(active_count)
        if not capacity_check["allowed"]:
            raise HTTPException(status_code=429, detail=capacity_check)

        with state_lock:
            cameraIDS[camID] = camID
            cameraFrameCount[camID] = 0

        start_camera_worker(camID, source, source_type)
        set_camera_state(
            cam_id=camID,
            source=camID,
            source_type=source_type,
    ...
    _record_audit_event(
        ...
        resource_type="camera",
        resource_id=camID,
        details={"source_type": source_type, "execution": "legacy"},
    )
    return {
...
def recoverAlerts(
    ...
    alerts = get_alerts_after_id(
        ...
        after_id=after_id,
        created_after=(
            created_after.astimezone(timezone.utc).replace(tzinfo=None)
            if created_after is not None and created_after.tzinfo is not None
            else created_after
        ),
        limit=limit,
