...
def cleanAlertMemory(timeout: float = 300):
    ...
    for key in list(alertMemory.keys()):
        if now - alertMemory[key] > timeout:
            del alertMemory[key]
...
def forget_snapshot_track(camID: str, track_id):
    prefix = f"{camID}_{track_id}_"
...
def crop_and_encode_snapshot(frame, box):
    try:
        ...
        print("crop_and_encode_snapshot error:", e)
        return None
...
def cleanAlertSnapshots(days: int = SNAPSHOT_RETENTION_DAYS):
    try:
        ...
        print("cleanAlertSnapshots error:", e)